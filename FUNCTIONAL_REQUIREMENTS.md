...
|----|-----------------|-----------|--------|
| FR-PUT-001 | `trash-put <path>...` SHALL move each argument to the `files/` subdirectory of the appropriate trash directory per the FreeDesktop.org Trash Specification | E1.1 | Implemented |
...
| FR-PUT-005 | `trash-put` SHALL resolve collisions in `files/` by appending a numeric suffix and SHALL use the same suffix for the corresponding `.trashinfo` file | E1.1 | Implemented |
...
| FR-PUT-009 | `trash-put` SHALL process all path arguments and SHALL NOT abort on the first failure; per-argument errors SHALL be printed to stderr and exit code SHALL be non-zero if any argument failed | E1.1 | Implemented |
| FR-PUT-010 | `trash-put` SHALL produce `trash-put: cannot trash '<path>': No such file or directory` for non-existent paths, not a Python traceback | E1.4 | Implemented |
| FR-PUT-011 | `trash-put` SHALL accept device files, sockets, named pipes, and symlinks without following symlink targets | E1.4 | Implemented |
...
| ID | SHALL Statement | Traces To | Status |
|----|-----------------|-----------|--------|
| FR-LIST-001 | `trash-list` SHALL scan all FreeDesktop-compliant trash directories accessible to the current user: `$XDG_DATA_HOME/Trash` and per-volume trash directories on all mounted filesystems | E2.1 | Implemented |
...
| FR-LIST-004 | `trash-list` SHALL parse malformed or missing `DeletionDate` fields without aborting; items with unparseable dates SHALL be listed with `<unknown date>` | E2.1 | Implemented |
...
| FR-RESTORE-005 | When the original path of an item to be restored is already occupied, `trash-restore` SHALL abort the restore of that item and print `trash-restore: cannot restore '<path>': destination already exists` | E3.2 | Implemented |
...
| FR-RESTORE-008 | Out-of-range or non-numeric selection input SHALL produce a descriptive error; the command SHALL NOT raise an unhandled exception | E3.1 | Implemented |
...
| FR-EMPTY-002 | `trash-empty <days>` SHALL permanently delete only items whose `DeletionDate` is more than `<days>` days before the current time | E4.2 | Implemented |
...
| FR-EMPTY-005 | `trash-empty --force` SHALL skip any interactive confirmation prompt regardless of the number of items | E4.2 | Implemented |

---

## FR-CORE — Rust Core Crate (`src/`)
...
| FR-CORE-003 | `TrashedItem` SHALL carry `original_path`, `trashed_path`, `info_path`, `trash_dir` as `PathBuf`, plus `size_bytes: Option<u64>` and `deleted_at: Option<DateTime<Utc>>` | E5.1 | Implemented |
| FR-CORE-004 | `TrashDirectory` SHALL carry `path`, `files_dir`, `info_dir` as `PathBuf` and `mount_point: Option<PathBuf>` | E5.1 | Implemented |
...
| FR-CORE-006 | `FileSystem` trait SHALL abstract all filesystem I/O so command implementations can be unit tested by injecting a fake implementation | E5.1 | Implemented |
...
| ID | SHALL Statement | Traces To | Status |
|----|-----------------|-----------|--------|
| FR-CONTRACT-001 | JSON Schema contracts for each command's request and response types SHALL reside in `contracts/` | E5.2 | Implemented |
| FR-CONTRACT-002 | The Python test suite SHALL validate actual bridge inputs and outputs against the JSON Schema contracts | E5.2 | Implemented |
...
| FR-COMPAT-005 | `trash-list` and `trash-put` SHALL resolve trash directories by scanning all accessible volume mount points | E1.3 | Implemented |
